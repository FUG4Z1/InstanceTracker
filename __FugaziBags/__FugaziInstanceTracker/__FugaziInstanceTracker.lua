...
local IT_SKIN = {
    ...
    pimp_purple = {
        -- Match __FugaziBAGS Pimp Purple exactly so tracker/ledger/items look like the inventory.
        mainBackdrop = {
            bgFile   = "Interface\\Tooltips\\UI-Tooltip-Background",
            edgeFile = "Interface\\Tooltips\\UI-Tooltip-Background",
            tile     = true, tileSize = 16, edgeSize = 1,
        ...
        -- Same base purple background as bags inventory (see-through; suede sits on top).
...
local function ApplyInstanceTrackerSkin(f)
    ...
    if s.mainBackdrop then
        f:SetBackdrop(s.mainBackdrop)
    end
    if s.mainBg then
        f:SetBackdropColor(unpack(s.mainBg))
    end
    if s.mainBorder then
        f:SetBackdropBorderColor(unpack(s.mainBorder))
    end

    -- Suede overlay only for Pimp Purple on tracker frames (same alpha as bags inventory).
...
local function InstallGPHInvHook()
    -- Do not override the bag key here anymore; __FugaziBAGS owns that behavior.
    return
end
...
local function CollapseInPlace(frame, collapsedHeight, isSnappedTo)
    ...
    if debug then
        DEFAULT_CHAT_FRAME:AddMessage("[Fugazi collapse] CollapseInPlace: frame=" .. tostring(frame:GetName() or frame) .. " targetH=" .. tostring(collapsedHeight) .. " combat=" .. tostring(inCombat) .. " heightBefore=" .. tostring(heightBefore) .. " anchor=" .. tostring(pt) .. " relTo=" .. tostring(relToName))
    end
    if pt and relTo and isSnappedTo and isSnappedTo(relTo) then
        frame:SetHeight(collapsedHeight)
        if debug then
            DEFAULT_CHAT_FRAME:AddMessage("[Fugazi collapse] CollapseInPlace: snapped path; heightAfter=" .. tostring(frame:GetHeight()))
        end
        return
    end
    local left, top = frame:GetLeft(), frame:GetTop()
    ...
    frame:SetHeight(collapsedHeight)
    if debug then
        DEFAULT_CHAT_FRAME:AddMessage("[Fugazi collapse] CollapseInPlace: re-anchor path; heightAfter=" .. tostring(frame:GetHeight()) .. " (wanted " .. tostring(collapsedHeight) .. ")")
    end
end
...
function FugaziInstanceTracker_RecordGPHRun(startTime, endTime, startGold, goldEarned, itemList, qualityCounts)
    ...
    AddonPrint(
        ColorText("[InstanceTracker] ", 0.4, 0.8, 1)
        .. "GPH session saved to Run Stats from _FugaziBAGS: " .. FormatTimeMedium(run.duration)
        .. " | " .. FormatGoldPlain(run.goldCopper)
    )
    if statsFrame and statsFrame:IsShown() and type(RefreshStatsUI) == "function" then
        RefreshStatsUI()
...
RefreshGPHUI = function()
    ...
    if #itemList == 0 then
        ...
        for idx, entry in ipairs(listToUse) do
            ...
            if not rowOk then
                AddonPrint("[Fugazi] GPH row " .. tostring(itemIdx) .. " error: " .. tostring(rowErr))
                yOff = yOff + 18
            end
            end
        end
